...
- [ ] dev/04 v1 Private-repo installs get enforced egress by default (`private_egress_guard` default-on or harden-runner block-mode equivalent), or the fail-open behavior is loudly declared in the install output. Prerequisite (audit §1.6): the flag's contract is fixed — `egress-guard.sh` moves into the substrate runtime (runner security, injected with the step) or compile fails when the flag is set and the profile doesn't carry the script; today only soc2-baseline carries it and any other flag-setter gets agent jobs that die on a missing file.
...
- [ ] dev/06 v1 The hold-label vocabulary has ONE source of truth (audit §1.1, §2.2): `merge.maintainer_block_labels` is read by `rearm-auto-merge.ts` (the substrate owns NO label vocabulary — `agent-paused` blocks merges because the profile lists it, not because the sweep hardcodes it) and consulted by the pm/reviewer skills; the `agent-blocked`-on-a-green-PR case (declared block label, auto-merge re-armed anyway) is fixed and fixtured.
...
- [ ] dev/09 v1 The merge boundary's script bodies are inside the deterministically gated scope (audit §1.5): `human_required_paths` covers the gate/sweep/reconcile scripts (wherever they live after OA-6 dev/07), so an agent PR editing the human gate's own qualification logic can no longer land on reviewer judgment alone.
...
- [ ] dev/06 v1 Every `policy.box` key passes the parameter-with-a-reader test (audit §2.1): keep/wire/delete per the disposition table (9 dead keys incl. the triplicated `human_required_topics`); a `check:policy-consumers` lint makes wire-or-delete an invariant; the dead `skills/open-autonomy-upgrade/` dir is removed; the stale `AGENTS.md` profile seed is refreshed from root (it still ships the dead `open-autonomy-*` glob); the unshipped maintainer skill is shipped or its dangling manifest reference dropped (audit §4.3); self-driving ships a `provision.json` seed or the INSTALL_OWNED entry is annotated as optional (audit §4.4).
- [ ] dev/07 v1 The CODE_HOST_RESOURCES split is completed at the script layer (audit §2.2): `rearm-auto-merge.ts`, `human-approval-gate.ts`, `reconcile-merged-issues.ts`, and `check-supply-chain.ts` move from the substrate's vendored runtime into profile-carried resources (the `profiles/self-driving/scripts/` + `resources:` mechanism); `agent-propose.ts`'s home is a recorded decision (migrate, or document it as the one capability-realization script the runner vendors); `bin/sync-runtime.ts`'s "mirror holds only substrate machinery" header becomes true; `docs/CODE_HOST_RESOURCES.md`'s Open/deferred section records the ruling. Pairs with OA-5 dev/09 (the moved scripts enter `human_required_paths`).
- [ ] dev/08 v1 The label-vocabulary residue is wired (audit §2.2): preflight's seeded label list derives from policy + contract constants instead of a fifth hand-kept copy; SPEC documents the contract/policy vocabulary split (status contexts, `human-required`, `agent-paused` are seam-contract constants; `merge.maintainer_block_labels` is org-tunable policy).
