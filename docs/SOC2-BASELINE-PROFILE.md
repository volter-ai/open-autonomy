...
| **M5** | **`policy.box.merge`** | `require_ci`, `require_low_risk_review`, `require_current_head_sha`, `maintainer_block_labels` | `profiles/self-driving/ir.yml` policy box; read by the merge gate. **Deterministic merge-gate config.** |
