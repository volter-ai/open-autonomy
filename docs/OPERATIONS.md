...
> (ztrack renamed the plugin dir `ztrack-gate` → `ztrack` in plugin 0.3.0; this repo's ztrack
> dev-dependency is pinned past that, and `bin/settings-merge.test.ts` fails if the wired path stops
> resolving against the installed ztrack — so the loop can't go silently dead on a future rename.)
>
