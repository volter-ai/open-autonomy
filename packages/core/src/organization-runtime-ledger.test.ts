...
describe('runtime proof-accounting ledger', () => {
  ...
  test('closes R11 only after live harness, sandbox, replay, and skeptical-review evidence', () => {
    const corpus = JSON.parse(readFileSync('docs/runtime-ledgers/r11-closure.json','utf8')) as RuntimeLedgerCorpus;
    expect(validateRuntimeLedger(corpus, expected, manifest.items)).toEqual([]);
    expect(corpus.checkpointStateLedger.find((entry) => entry.id === 'R11')?.status).toBe('complete');
    expect(['R15','R16'].every((id) => corpus.checkpointStateLedger.find((entry) => entry.id === id)?.status === 'ready')).toBe(true);
    expect(corpus.residualLedger).toEqual([]);
    expect(corpus.obligationLedger.filter((entry) => entry.checkpoint === 'R11').every((entry) => entry.assurance === 'property-tested' && entry.evidence.includes('ev-r11-review'))).toBe(true);
  });

  test('closes R12 only after official SDK interoperation and adversarial protocol evidence', () => {
    const corpus = JSON.parse(readFileSync('docs/runtime-ledgers/r12-closure.json','utf8')) as RuntimeLedgerCorpus;
    expect(validateRuntimeLedger(corpus, expected, manifest.items)).toEqual([]);
    expect(corpus.checkpointStateLedger.find((entry) => entry.id === 'R12')?.status).toBe('complete');
    expect(corpus.residualLedger).toEqual([]);
    expect(corpus.obligationLedger.filter((entry) => entry.checkpoint === 'R12').every((entry) => entry.assurance === 'property-tested' && entry.evidence.includes('ev-r12-review'))).toBe(true);
  });

  test('closes R15 only after isolated live Hermes lifecycle and fault evidence', () => {
    const corpus = JSON.parse(readFileSync('docs/runtime-ledgers/r15-closure.json','utf8')) as RuntimeLedgerCorpus;
    expect(validateRuntimeLedger(corpus, expected, manifest.items)).toEqual([]);
    expect(corpus.checkpointStateLedger.find((entry) => entry.id === 'R15')?.status).toBe('complete');
    expect(corpus.residualLedger).toEqual([]);
  ...
  test('closes R18 only after authenticated replay, recovery, privacy, and skeptical-review evidence', () => {
    const corpus = JSON.parse(readFileSync('docs/runtime-ledgers/r18-closure.json','utf8')) as RuntimeLedgerCorpus;
    expect(validateRuntimeLedger(corpus, expected, manifest.items)).toEqual([]);
    expect(corpus.checkpointStateLedger.find((entry) => entry.id === 'R18')?.status).toBe('complete');
    expect(corpus.residualLedger.filter((entry) => entry.disposition === 'open')).toEqual([]);
  ...
  test('closes R20 with typed command properties and a real SDK over the pinned Slack service twin', () => {
    const corpus = JSON.parse(readFileSync('docs/runtime-ledgers/r20-closure.json','utf8')) as RuntimeLedgerCorpus;
    expect(validateRuntimeLedger(corpus, expected, manifest.items)).toEqual([]);
    expect(corpus.checkpointStateLedger.find((entry) => entry.id === 'R20')?.status).toBe('complete');
    expect(corpus.checkpointStateLedger.find((entry) => entry.id === 'R21')?.status).toBe('ready');
    expect(corpus.residualLedger.filter((entry) => entry.checkpoint === 'R20' && entry.disposition === 'open')).toEqual([]);
    expect(corpus.obligationLedger.filter((entry) => entry.checkpoint === 'R20').every((entry) =>
  ...
  test('closes R24 only with matched actual pinned native substrate receipts',()=>{const c=JSON.parse(readFileSync('docs/runtime-ledgers/r24-closure.json','utf8'))as RuntimeLedgerCorpus;expect(validateRuntimeLedger(c,expected,manifest.items)).toEqual([]);expect(c.checkpointStateLedger.find(x=>x.id==='R24')?.status).toBe('complete');expect(c.checkpointStateLedger.find(x=>x.id==='R25')?.status).toBe('ready');expect(c.obligationLedger.filter(x=>x.checkpoint==='R24').every(x=>x.assurance==='live-observed'&&x.evidence.includes('ev-r24-native'))).toBe(true);});
  test('closes R25 only within the calibrated R23/R24 twin world',()=>{const c=JSON.parse(readFileSync('docs/runtime-ledgers/r25-closure.json','utf8'))as RuntimeLedgerCorpus;expect(validateRuntimeLedger(c,expected,manifest.items)).toEqual([]);expect(c.checkpointStateLedger.find(x=>x.id==='R25')?.status).toBe('complete');expect(c.checkpointStateLedger.find(x=>x.id==='R26')?.status).toBe('ready');});
  test('closes R26 with replayed constraints and independent certificate',()=>{const c=JSON.parse(readFileSync('docs/runtime-ledgers/r26-closure.json','utf8'))as RuntimeLedgerCorpus;expect(validateRuntimeLedger(c,expected,manifest.items)).toEqual([]);expect(c.checkpointStateLedger.find(x=>x.id==='R26')?.status).toBe('complete');expect(c.checkpointStateLedger.find(x=>x.id==='R27')?.status).toBe('ready');});
  test('closes R27 with preregistered designs and actual pinned rollback canary',()=>{const c=JSON.parse(readFileSync('docs/runtime-ledgers/r27-closure.json','utf8'))as RuntimeLedgerCorpus;expect(validateRuntimeLedger(c,expected,manifest.items)).toEqual([]);expect(c.checkpointStateLedger.find(x=>x.id==='R27')?.status).toBe('complete');expect(c.checkpointStateLedger.find(x=>x.id==='R28')?.status).toBe('ready');});
  test('closes R28 with repository effects and fresh OS reconstruction at every phase',()=>{const c=JSON.parse(readFileSync('docs/runtime-ledgers/r28-closure.json','utf8'))as RuntimeLedgerCorpus;expect(validateRuntimeLedger(c,expected,manifest.items)).toEqual([]);expect(c.checkpointStateLedger.every(x=>x.status==='complete')).toBe(true);expect(c.obligationLedger.filter(x=>x.checkpoint==='R28').every(x=>x.evidence.includes('ev-r28-restarts')&&x.evidence.includes('ev-r28-review'))).toBe(true);});
