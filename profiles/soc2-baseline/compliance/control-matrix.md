...
| C3 | Egress lockdown (exfiltration prevention) | CC6 | `harden-runner egress-policy: block` on every credentialed job. **ENFORCEMENT REQUIRES the StepSecurity GitHub App** (eBPF blocking): verified live that without the app harden-runner runs in monitor/audit mode and ALLOWS non-allowlisted egress (fails open, on public + private). Install the app to make the block real. | harden-runner step + run logs; StepSecurity dashboard (with app) |
