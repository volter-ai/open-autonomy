...
| C3 | Egress lockdown (exfiltration prevention) | CC6 | `harden-runner egress-policy: block` on every credentialed job — enforced by the **free Harden-Runner Action, NO StepSecurity account/App** on hosted runners. The allowlist MUST include harden-runner's own agent endpoints (`agent.api.stepsecurity.io`, `prod.app-api.stepsecurity.io`) or the block agent can't initialize and fails open; the shipped allowlist now includes them. **Verified live (public repo, no account): non-allowlisted egress to example.com DENIED (curl exit 7); allowlisted api.github.com 200.** | harden-runner step + run logs (curl exit 7 on a blocked host) |
