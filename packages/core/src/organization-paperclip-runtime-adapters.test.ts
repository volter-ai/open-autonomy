...
describe("R16 production Paperclip runtime ownership adapter",()=>{
  ...
  test("composes actual provider, ingestor, sidecar, and bridge through restore, rollback, and teardown",()=>{
    ...
    const pin={repository:"repo",commit,treeDigest:sha("tree"),lockDigest:sha("lock"),executableDigest:sha("source"),builtExecutableDigest:sha("built"),dependencyDigest:sha("deps")};
    ...
    const observation=()=>({running:lifecycleState.status==="running",pid:lifecycleState.status==="running"?42:null,endpoint:baseUrl.slice(0,-1),dataDir:join(root,"data"),pin,healthy:lifecycleState.status==="running"});
