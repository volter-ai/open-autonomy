...
export const ORGANIZATION_SEMANTIC_COVERAGE: readonly SemanticCoverageEntry[] = [
  ...
  entry('AssurancePolicy', 'minimum allowApproximation acceptedAssumptions asOf maxEvidenceAgeMs', 'compiler', 'implemented', 'risk-indexed minimum evidence, freshness, and approximation policy', 'P7'),
  ...
  entry('DeploymentExtension', 'regions tenants dataResidency credentials upgradeFrom', 'compiler', 'implemented', 'provider deployment properties used by hard constraints', 'P7'),
  entry('DeploymentMigration', 'id kind from to lossy sourcePaths rollback', 'compiler', 'implemented', 'finite path-accounted reversible migration graph edge declaration', 'P7'),
  entry('DeploymentFiniteDomain', 'completeness maxAssignments maxInstancesPerManifest constraints allowedAdapters migrations preferredManifests', 'compiler', 'implemented', 'explicit finite deployment CSP domain', 'P7'),
  entry('ConstraintWitness', 'constraint class providers evidence assumptions detail', 'compiler', 'implemented', 'constructive hard-constraint satisfaction witness', 'P7'),
  entry('ObjectiveCoordinate', 'measure unit per attribution value uncertainty effectiveAt evidence assumptions', 'compiler', 'implemented', 'dimensionally keyed evidence-bound Pareto coordinate', 'P7'),
  entry('ConstructivePlan', 'adapters migrations', 'compiler', 'implemented', 'endpoint-specific adapter and ordered migration plan', 'P7'),
  entry('DeploymentCertificate', 'domainCardinality explored complete constraintWitnesses semanticObligations plan objective', 'compiler', 'implemented', 'replayable SAT and finite-search certificate', 'P7'),
  entry('CertifiedDeploymentCandidate', 'certificate', 'compiler', 'implemented', 'feasible candidate with independently checkable certificate', 'P7'),
  entry('DeploymentSolverResult', 'status frontier explored domainCardinality complete unsatisfiedCore coreMinimality errors', 'compiler', 'implemented', 'Pareto SAT, finite UNSAT, bounded exhaustion, or invalid input result', 'P7'),
  entry('DeploymentCertificateEvidence', 'id assurance observedAt expiresAt acceptance', 'compiler', 'implemented', 'certificate-pinned evidence and acceptance record', 'P7'),
  entry('DeploymentObjectiveDimension', 'key direction unit horizon', 'compiler', 'implemented', 'objective dimension declaration preserving comparison semantics', 'P7'),
  entry('DeploymentObjectiveValue', 'value unit horizon uncertainty evidence', 'compiler', 'implemented', 'evidence-bound uncertain objective observation', 'P7'),
  entry('DeploymentConstraintWitness', 'constraint satisfied evidence', 'compiler', 'implemented', 'independent certificate hard-constraint witness', 'P7'),
  entry('DeploymentArtifactWitness', 'artifact obligations evidence', 'compiler', 'implemented', 'adapter or migration obligation witness', 'P7'),
  entry('PlanningCertificateCandidate', 'id hardConstraints objectives adapters migrations adapterWitnesses migrationWitnesses', 'compiler', 'implemented', 'standalone planning-certificate frontier member', 'P7'),
  entry('DeploymentPlanningCertificate', 'schema generatedAt search hardConstraints sourceObligations objectiveDimensions knownAdapters knownMigrations evidence frontier', 'compiler', 'implemented', 'standalone independently verifiable planning certificate', 'P7'),
  entry('DeploymentCertificateVerification', 'valid errors', 'compiler', 'implemented', 'independent certificate verification result', 'P7'),
  entry('DeploymentBundleInput', 'schema canonicalOrganization organizationDigest lock compiler requiredObligations selectedManifests loweringCertificates nativeArtifacts policies secretReferences migrations healthProbes rollbackPlan expectedObservations sbom provenance', 'compiler', 'implemented', 'complete canonical immutable deployment input including standards-conformant supply-chain and operational contracts', 'P8'),
  entry('DeploymentBundleManifest', 'schema organizationDigest compilerDigest semanticDigest inventory', 'compiler', 'implemented', 'content-addressed semantic and artifact inventory root', 'P8'),
  entry('DeploymentBundle', 'manifest bytes digest', 'compiler', 'implemented', 'canonical reproducible deployment payload and its immutable address', 'P8'),
  entry('DeploymentBundleSigner', 'id algorithm sign', 'compiler', 'implemented', 'identified signing capability for bundle and deployment attestations', 'P8'),
  entry('SignedDeploymentBundle', 'bundle signatures', 'compiler', 'implemented', 'immutable bundle with independently verifiable trust-policy signatures', 'P8'),
  entry('DeploymentBundleTrustPolicy', 'requiredSigners trustedSigners rejectUnknownArtifacts rejectSecrets', 'compiler', 'implemented', 'closed fail-safe bundle and attestation verification policy', 'P8'),
  entry('DeploymentRelease', 'schema bundleDigest semanticDigest environment bindingsDigest releaseDigest signatures', 'observation', 'implemented', 'environment promotion attestation without semantic recompilation', 'P8'),
  entry('LiveDeploymentInstance', 'schema instanceId tenant observedAt bundleDigest semanticDigest organizationDigest compilerDigest componentDigests releaseDigest environment attestationDigest signatures', 'observation', 'implemented', 'immutable running-instance trace to exact release and compilation inputs', 'P8'),
  entry('SignedTypedLossAuthorization', 'statement digest signer algorithm signature', 'compiler', 'implemented', 'externally verified replay-protected authorization for one migration loss', 'P9'),
  entry('LegacyV1Installation', 'schema version installationId profile constructs publicCommands ownedState', 'dialect-bound', 'implemented', 'versioned complete legacy installation migration input', 'P9'),
  entry('MigrationCorpusEntry', 'id source declaredObservations', 'compiler', 'implemented', 'one real migration source with independently projected observable behavior', 'P9'),
  entry('DiscoveredMigrationCorpusEntry', 'v1Ir provenance', 'compiler', 'implemented', 'repository-discovered validated v1 profile with content provenance', 'P9'),
  entry('DiscoveredV2Compiler', 'id generation digest sourceDigests catalogDigest runtimeAbi', 'compiler', 'implemented', 'content-identified native v2 migration compiler', 'P9'),
  entry('LegacyMigrationResult', 'organization frontend ledger retainedDialects', 'compiler', 'implemented', 'strictly verified v1-to-v2 result with total construct disposition', 'P9'),
  entry('MigrationCorpusReport', 'profiles equivalent untriagedResiduals corpusDigest digest compilerProvenance assertReadyForShadow', 'compiler', 'implemented', 'dual-compiled per-observation differential and residual report', 'P9'),
  entry('SignedMigrationCutoverEvidence', 'statement digest signer algorithm signature', 'observation', 'implemented', 'signed corpus- and prior-state-bound staged cutover evidence', 'P9'),
  entry('MigrationCutoverController', 'state journal stateDigest mutationHandlerRegistryDigest transition recordOwnedMutation snapshot', 'observation', 'implemented', 'durable staged migration controller with externally trusted replay and rollback translation', 'P9'),
  entry('LegacyRemovalDecision', 'removable criteria evidenceDigest digest', 'compiler', 'implemented', 'externally derived immutable legacy-removal decision', 'P9'),
  ...
  entry('V1ExecutionLoweringOptions', 'targets codeHost policy resources documents actorProjection', 'compiler', 'implemented', 'bounded native v1 emission environment', 'P8'),
