...
| supercode HEAD | **MOVING** — a live parallel UX workflow pushes to supercode main continuously (studied `c616803` → `add49ca` → `c062940`+ within this session; fresh `ux/*` branches active). Skeptic-2 finding 2: any frozen HEAD number here goes stale in hours; the durable facts are FILE-level | install-relevant files verified byte-unchanged across the full drift range: CI job display name `test + clippy + fmt` (`supercode/.github/workflows/ci.yml:12-13`); `package.json` = ztrack `1.0.0` only (`supercode/package.json:4`); tracker-config unchanged (`supercode/.volter/tracker-config.json`: backend `markdown`, teamKey `SUP`, validation installedFrom `simple-sdlc`). CONSEQUENCE (skeptic-2 fix): U7 and U10 re-verify these three files + rebase at execution time; U13's board mutations must respect the live workflow (see U13) |
...
| OA `agent-review` | posted by `open-autonomy/.github/workflows/reviewer.yml` — `pull_request_target` trigger (`:8`); job gate `vars.PUBLIC_AGENT_REPO_PAUSED != 'true'` + fires for same-repo or OWNER/MEMBER/COLLABORATOR PRs (`:34`). **Skeptic-2 finding 1 (BLOCKER, live-proven on PR #127):** the reviewer skill reviews ONLY canonical `agent/issue-*` branches — "for anything else, post failure and comment that human review is required" (`open-autonomy/.claude/skills/reviewer/SKILL.md:30`; mirrored in `.codex/skills/reviewer/SKILL.md:30`); PR #123 (branch `adoption-fixes-backlog`) carried that same red status and was merged by a human ADMIN past it — precedent for admin bypass, not for a non-admin green-merge | landing PRs from this session MUST ride canonical `agent/issue-<N>` branches bound to a real linked OA issue (the reviewer then reviews on the merits, reading the linked issue's labels — `reviewer/SKILL.md:22-26`); a FAIL verdict on the merits = rework the PR; if the reviewer fails for POLICY (not merit) reasons even on a canonical branch, the fallback is maintainer escalation, recorded in the honesty ledger |
...
| Unit | Status | Detail |
|---|---|---|
| **U1+U2** | **DONE @ `7e9e0bd`** (branch `simple-gh-profile`; all 8 Fable-review findings amended in; `bun run check` exit 0, lint clean, kit compiles) | Review findings applied: `provision.json` `required_secrets`/`variables`/`labels` keys; `agent-visual-verify.ts` in the human-required set; Scope/untrusted-instructions section; sha-bound `oa-review` verdicts; 4 minors. |
| U3 | **in flight — rev 3 canonical-branch flow** | First attempt PR #127 (branch `simple-gh-profile`) drew the live `agent-review: fail — Non-canonical branch` (skeptic-2 finding 1); relanding as `agent/issue-<N>` + linked issue per the amended contract; #127 to be closed with cross-reference. |
| U5–U15 | pending | All specified in full below; none started. |
| Spec ledger | this doc @ supercode main (`docs/oa-install/OA-SIMPLE-GH-INSTALL-MAXIMAL-SPEC.md`, first cut `5f95959`) + OA branch `oa-supercode-install-spec` (`docs/OA-SUPERCODE-INSTALL-SPEC.md`, first cut `3453933`) | Skeptic-3 (fresh, independent, on the committed bytes) returned REFUTED with 4 findings — 1 BLOCKER (the canonical-branch wall), 1 MAJOR (HEAD drift/coexistence), 2 MINOR (`:208` cite, deputy-authority quote) — ALL applied in this revision. OA-branch copy updated in the same push. |
...
**Contract (rev 3, skeptic-2 finding 1 applied — the canonical-branch flow).** (1) File a real GitHub issue on volter-ai/open-autonomy describing the profile work (what/why/ACs — the PR body content; no hold labels, no `agent-develop-only`). (2) Cut branch `agent/issue-<N>` at the reviewed profile commit and push. (3) Open the PR from that canonical branch, linking the issue (`Closes #<N>`). (4) The reviewer then reviews ON THE MERITS (`reviewer/SKILL.md:22-26` — reads the diff, the linked issue's labels, constitution/invariants/rubric). Wait for `ci` + `agent-review` + `human-approval` green. If `agent-review` FAILS on the merits: treat as a real review, rework (≤3 attempts), re-push, expect re-review + a fresh sha-current verdict. (5) Merge via `gh pr merge --squash` (non-admin merge is legal once required checks are green). NEVER `--admin`, never direct-push main. Fallback (policy-fail on a canonical branch, or any other non-merit wall): maintainer escalation recorded in the honesty ledger — the PR stays open, green on every satisfiable check, with the escalation note. The original non-canonical PR #127 is closed with a cross-reference comment. Loop-interference guard: the linked issue is created immediately before the PR (minutes apart) so OA's own PM never sees an unclaimed open issue without an in-flight PR; if OA's loop nevertheless dispatches a duplicate developer, close whichever PR is later, keep ours, note it.
...
- The known stale doc text to fix in the same PR: the runner-deps line instructs `termfleet` only (`open-autonomy/docs/INSTALL-AGENT.md:208`) while the emitted backend bare-imports `@termfleet/core/local-providers.js` (`open-autonomy/packages/substrate-local/src/backend.mjs:8,16-17`) and twin explicitly added both deps (`twin@a0b2aad` commit message: "add termfleet + @termfleet/core runner deps") — F-C/OA-19 candidate.
...
- The deputy authority (skeptic-2 finding 4 fix — quoted verbatim so the deputization is auditable): the session brief states "AUTONOMY: nobody is here to answer you. Push to 100% complete + proven. Never stop to ask/hold/hand a fork back; at every reversible fork make the best call, write one line of rationale, and PROCEED." Board state changes are reversible forks; the operator-review fence (OA-07, `emit.ts:106-113`) is therefore executed BY the deputy, with the full pre-mutation board state recorded for operator review after the fact.
- Study: §4 step 8 (board review + unpause), NEXT STEPS 9 (operator scopes the first wave).

**Contract.** Acting as operator's deputy (autonomy directive; rationale recorded): scope the first wave to ONE small, low-risk, well-specified open item from the board's priority order — prefer a tiny PARITY correctness item with crisp ACs. Scoping is by STATUS, not the frontier: set the chosen item to `ready` state and ensure it is the ONLY `ready`-state issue. Coexistence rule (skeptic-2 finding 2): supercode's board is actively used by a live parallel UX workflow — record the full pre-mutation board state (`ztrack issue list` snapshot committed as evidence), demote pre-existing `ready` items ONLY if their namespace is not actively being worked by the live workflow (an in-flight `ux/*` branch or fresh commits naming the item = actively worked → do NOT touch it; instead pick the wave item such that the manager's dispatch cannot collide, and record why the only-ready-issue AC is satisfied within the scoped namespace); every demotion is recorded + reversible. The U7 manager doctrine (`standards/workflow.md` + SKILL.md) must say "dispatch ONLY `ready`-state issues" (U13 verifies presence). Then `rm .open-autonomy/paused`; commit+push the scoping.
...
| docs instruct `termfleet` only | `INSTALL-AGENT.md:207-209` | `INSTALL-AGENT.md:208` |
