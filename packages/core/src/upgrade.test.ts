...
describe('upgrade = recompile (regenerate derived, keep owned inputs, prune orphans)', () => {
  ...
  // OA-10 (c): planUpgrade/applyUpgrade must apply the SAME merge policy the fresh-compile CLI does for
  // `.claude/settings.json`, or every upgrade would revert an adopter's merged file back to the profile's
  // whole-file copy (AC-7). A trivial JSON key-merge strategy stands in for the real one (bin/settings-merge.ts
  // is the CLI's own unit coverage) — this describes core's contract with the strategy map, not the JSON shape.
  describe('mergeStrategies (OA-10c — upgrade never reverts a merged file)', () => {
      ...
      'settings.json': {
        merge(existing, generated) {
          try {
            ...
            return { content: JSON.stringify({ ...e, ...g }), note: 'merged' };
          } catch {
            return undefined;
          }
        },
    ...
    test('a byte-differing file WITH a merge strategy is planned as update, and applyUpgrade writes the MERGED content (not the raw generated bytes)', () => {
      const { profile, target } = dirs();
      write(target, 'settings.json', '{"adopterKey":"mine"}');
      const out: CompileOutput = { generated: { 'settings.json': '{"ourHook":"stop"}' }, copies: [] };
      const plan = planUpgrade(out, profile, target, { mergeStrategies: jsonMergeStrategies });
      expect(plan.changes).toContainEqual({ path: 'settings.json', action: 'update' });
      applyUpgrade(plan, out, profile, target, jsonMergeStrategies);
      expect(JSON.parse(readFileSync(join(target, 'settings.json'), 'utf8'))).toEqual({ adopterKey: 'mine', ourHook: 'stop' });
    });

    test('a file ALREADY merged (its content, re-merged, is unchanged) reads as up to date — no `update` planned', () => {
      const { profile, target } = dirs();
      // The adopter's file already contains everything a merge would produce — re-merging it against
      // itself is idempotent, so planUpgrade must not schedule a pointless `update`.
      write(target, 'settings.json', '{"adopterKey":"mine","ourHook":"stop"}');
      const out: CompileOutput = { generated: { 'settings.json': '{"ourHook":"stop"}' }, copies: [] };
      const plan = planUpgrade(out, profile, target, { mergeStrategies: jsonMergeStrategies });
      expect(plan.changes.find((c) => c.path === 'settings.json')).toBeUndefined();
    });

    test('revert the strategy (omit mergeStrategies): the SAME adopter file reverts to an ordinary clobbering update', () => {
      // Tamper probe: without the strategy, planUpgrade/applyUpgrade fall back to their pre-OA-10 behavior
      // — a plain overwrite that DISCARDS the adopter's own key. This is what AC-7 exists to prevent.
      const { profile, target } = dirs();
      write(target, 'settings.json', '{"adopterKey":"mine"}');
      const out: CompileOutput = { generated: { 'settings.json': '{"ourHook":"stop"}' }, copies: [] };
      const plan = planUpgrade(out, profile, target); // no mergeStrategies
      expect(plan.changes).toContainEqual({ path: 'settings.json', action: 'update' });
      applyUpgrade(plan, out, profile, target); // no mergeStrategies
      expect(readFileSync(join(target, 'settings.json'), 'utf8')).toBe('{"ourHook":"stop"}'); // adopterKey LOST
    });

    test('an unmergeable existing file (invalid JSON) falls through to an ordinary overwrite even with a strategy passed', () => {
      const { profile, target } = dirs();
      write(target, 'settings.json', 'not valid json');
      const out: CompileOutput = { generated: { 'settings.json': '{"ourHook":"stop"}' }, copies: [] };
      const plan = planUpgrade(out, profile, target, { mergeStrategies: jsonMergeStrategies });
      expect(plan.changes).toContainEqual({ path: 'settings.json', action: 'update' });
      applyUpgrade(plan, out, profile, target, jsonMergeStrategies);
      expect(readFileSync(join(target, 'settings.json'), 'utf8')).toBe('{"ourHook":"stop"}');
    });
  });
});
