...
const paths = ["scripts/generate-r22-readiness-evidence.ts",
  ...
  "packages/core/src/organization-r22-external-campaign.ts", "packages/core/src/organization-r22-external-campaign.test.ts",
  "packages/core/src/organization-r22-human-calibration-cli.ts", "packages/core/src/generated/r22-human-calibration-study.schema.json",
  "bench/dev/evidence/verify-external-campaign.ts", "bench/dev/evidence/verify-external-campaign.test.ts",
  "bench/dev/evidence/r22-acquisition.ts", "bench/dev/evidence/r22-acquisition-cli.ts", "bench/dev/evidence/r22-acquisition.test.ts",
  "docs/evidence/R22-ACQUISITION-SKEPTICAL-REVIEW.md",
...
const evidence={checkpoint:"R22",purpose:"machine-reviewable benchmark-model, local-custody-fixture, and external-campaign-verifier readiness; never externally trusted benchmark closure",closureClaim:false,
  components:paths.map(path=>({path,sha256:sha(readFileSync(path))})),evidenceClasses:["deterministic-model","owned-local-fixture","external-evidence-acquisition","external-evidence-intake","external-evidence-verifier"],
  proves:["workload partitions and signed bundle mechanics are implemented","assigned trial omission is rejected by the model","local bubblewrap fixture denies unmounted hidden files and network","simulator and real-human result labels are separated","human calibration refuses an absent or single-rater matrix","external campaign matrices, authority separation, seeded replay, causal enrollment, statistics, and cost conservation are verified","external evidence intake requires an independently attested exact trust module and emits a timestamp-bound content-addressed receipt","external acquisition derives every registered human, trial, attack, rating, and cost cell; binds causal predecessors and distinct authorities; survives restart; and assembles the exact verifier input"],
