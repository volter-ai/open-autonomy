...
const calculus=()=>freezeU3ObservationCalculus(u3Fixture());
...
test("rejects chronology conflict and freeze attacks",()=>{rejects(v=>v.chronology[0].frozenAt="2027-02-01T00:00:00.000Z",/chronology/);rejects(v=>v.freezeReceipt="00",/chronology|freeze receipt/);rejects(v=>v.conflicts[0].leftValueDigest=v.conflicts[0].rightValueDigest,/conflict/);rejects(v=>v.conflicts[0].adjudicationReceipt="00",/adjudicator/);});
test("enforces floors taxonomy native schemas and exact reachability",()=>{rejects(v=>{const f=v.facts.find((x:any)=>x.criticality==="critical");f.criticality="noncritical";},/fact|conflict/);rejects(v=>v.facts.find((x:any)=>x.taxonomy==="omissions").absence=null,/fact|coherence/);rejects(v=>{const f=v.facts.find((x:any)=>x.semantic.domain==="model");f.semantic.domain="prompt";f.factReceipt=mac("a-semantic","u4-fact",{...f,factReceipt:undefined,criticalityEvidence:{...f.criticalityEvidence,receipt:undefined}});},/subdomain/);rejects(v=>v.nativeSchemas[0].version="2",/native schema|join/);rejects(v=>v.facts[0].default.valueJson='"wrong"',/shape/);rejects(v=>v.dimensions.pop(),/closed world/);rejects(v=>{const p={...v.provenance[0],id:"surplus"},body={id:p.id,sourceId:p.sourceId,nativeSchemaId:p.nativeSchemaId,kind:p.kind,sourceVersion:p.sourceVersion,mediaType:p.mediaType,acquiredAt:p.acquiredAt,sha256:p.sha256};p.bodyReceipt=mac(p.producerAuthorityId,"u4-provenance-body",body);p.custodyReceipt=mac(p.custodyAuthorityId,"u4-provenance-custody",{...body,producerAuthorityId:p.producerAuthorityId,bytesBase64:p.bytesBase64});v.provenance.push(p);},/surplus/);});
test("verifier redigests and resource bounds reject",()=>{const [v,c,r,t]=valid(),f:any=freezeU4SourceInventory(v,c,r,t),copy:any=structuredClone(f);copy.digest=H("bad");expect(()=>verifyFrozenU4SourceInventory(copy,c,r,t)).toThrow(/digest/);rejects(v=>v.facts[0].denotation="x".repeat(1_048_577),/field|byte/);});
test("compiled registry anchor rejects a consistently redigested population",()=>{const [,c,r,t]=valid(),x:any=structuredClone(r),{digest:_,...body}=x;void _;body.sources[0].nativeVersion="2";x.sources=body.sources;x.digest=computeU4SyntheticSourceRegistryDigest(body);expect(()=>freezeU4SourceInventory(inventory(),c,x,t)).toThrow(/independent registry fixture|registry/);});
test("trusted keys reject rekey-resign and producer-custodian role alias",()=>{rejects((v:any)=>{const a=v.authorities.find((x:any)=>x.id==="a-spec"),k=Buffer.alloc(32,7);a.verificationKeyDigest=H(k);a.trustRootSha256=H(k);for(const p of v.provenance.filter((x:any)=>x.producerAuthorityId==="a-spec")){const body={id:p.id,kind:p.kind,sourceVersion:p.sourceVersion,mediaType:p.mediaType,acquiredAt:p.acquiredAt,sha256:p.sha256};p.bodyReceipt=createHmac("sha256",k).update("u4-provenance-body\0").update(C(body)).digest("hex");}},/trusted key/);rejects(v=>{const p=v.provenance[0];p.custodyAuthorityId=p.producerAuthorityId;},/provenance/);});
test("conflict redigest criticality floor schema resign and invalid dates reject",()=>{rejects(v=>{const c=v.conflicts[0];c.leftProposition="rewritten";c.leftValueDigest=computeU4PropositionDigest(c.leftProposition);},/adjudicator|conflict join/);rejects(v=>{const f=v.facts.find((x:any)=>x.criticality==="critical");f.criticality="noncritical";f.criticalityEvidence.receipt=mac("a-semantic","u4-criticality",{factId:f.id,criticality:f.criticality,rationale:f.criticalityEvidence.rationale,evidenceProvenanceId:f.criticalityEvidence.evidenceProvenanceId,sourceFloor:f.criticalityEvidence.sourceFloor});},/fact|receipt/);rejects(v=>{const s=v.nativeSchemas[0],b=Buffer.from('{"type":"string"}');s.valueShape={type:"string"};s.schemaBytesBase64=b.toString("base64");s.schemaSha256=H(b);s.semanticSchemaDigest=computeU4NativeSchemaDigest(s.valueShape);s.producerReceipt=mac("a-schema","u4-native-schema",{id:s.id,version:s.version,sourceId:s.sourceId,pathPrefix:s.pathPrefix,schemaSha256:s.schemaSha256,semanticSchemaDigest:s.semanticSchemaDigest});s.custodyReceipt=mac("a-custody","u4-native-schema-custody",{id:s.id,schemaBytesBase64:s.schemaBytesBase64,schemaSha256:s.schemaSha256,producerAuthorityId:s.producerAuthorityId});const p=v.provenance.find((x:any)=>x.kind==="native-schema");p.bytesBase64=b.toString("base64");p.byteLength=b.length;p.sha256=H(b);const body={id:p.id,sourceId:p.sourceId,nativeSchemaId:p.nativeSchemaId,kind:p.kind,sourceVersion:p.sourceVersion,mediaType:p.mediaType,acquiredAt:p.acquiredAt,sha256:p.sha256};p.bodyReceipt=mac(p.producerAuthorityId,"u4-provenance-body",body);p.custodyReceipt=mac(p.custodyAuthorityId,"u4-provenance-custody",{...body,producerAuthorityId:p.producerAuthorityId,bytesBase64:p.bytesBase64});},/shape/);rejects(v=>v.provenance[0].acquiredAt="2026-02-30T00:00:00.000Z",/provenance/);rejects((v,t)=>t.chronology.frontendOutcomeNotBefore="not-a-date",/chronology/);});
test("taxonomy coherence is per source and catches a second-source context gap",()=>{const facts:any[]=inventory().facts.flatMap(f=>[f,{...structuredClone(f),sourceId:"source-b"}]);expect(()=>verifyU4TaxonomyCoherence(["source","source-b"],facts)).not.toThrow();const i=facts.findIndex(f=>f.sourceId==="source-b"&&f.semantic.domain==="model");facts.splice(i,1);expect(()=>verifyU4TaxonomyCoherence(["source","source-b"],facts)).toThrow(/subdomain/);});
test("source custody owner schema path and extraction topology resist substitution",()=>{
  rejects(v=>{const p=v.provenance.find((x:any)=>x.kind==="source-behavior");p.sourceId="source-b";const body={id:p.id,sourceId:p.sourceId,nativeSchemaId:p.nativeSchemaId,kind:p.kind,sourceVersion:p.sourceVersion,mediaType:p.mediaType,acquiredAt:p.acquiredAt,sha256:p.sha256};p.bodyReceipt=mac(p.producerAuthorityId,"u4-provenance-body",body);p.custodyReceipt=mac(p.custodyAuthorityId,"u4-provenance-custody",{...body,producerAuthorityId:p.producerAuthorityId,bytesBase64:p.bytesBase64});},/provenance join/);
  rejects(v=>v.sources[0].frontendOwnerId="wrong-frontend",/source authority owner/);
  rejects((v,t)=>{const a=v.authorities.find((x:any)=>x.id==="a-freezer"),k=t.authorityKeys.find((x:any)=>x.authorityId==="a-freezer");a.ownerId="implementer";k.ownerId="implementer";},/source independence|source authority/);
  rejects(v=>{const s=structuredClone(v.nativeSchemas[0]);s.id="native2";s.producerReceipt=mac("a-schema","u4-native-schema",{id:s.id,version:s.version,sourceId:s.sourceId,pathPrefix:s.pathPrefix,schemaSha256:s.schemaSha256,semanticSchemaDigest:s.semanticSchemaDigest});s.custodyReceipt=mac("a-custody","u4-native-schema-custody",{id:s.id,schemaBytesBase64:s.schemaBytesBase64,schemaSha256:s.schemaSha256,producerAuthorityId:s.producerAuthorityId});v.nativeSchemas.push(s);},/native schema provenance/);
