...
- The box keys you own and must literally act on, all under `.open-autonomy/autonomy.yml`'s
  ...
  - `risk.human_required_paths` — mechanically matchable protected paths; see §8.
...
4. Merge (`gh pr merge --squash`) **only when both** hold:
   ...
   - **Rework**: relaunch an implementation subagent with the failure, counted via an `oa-rework:
     ...
     the two-rework limit defined by this skill.
   - **Escalate**: at or above two rework attempts, or the failure is unclear/repeating — stop,
...
Any change touching a path in `risk.human_required_paths` or a semantic topic listed in
