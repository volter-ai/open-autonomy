...
const campaignId = "organization-universality-2026-v6",
  cutoff = "2026-07-23T23:59:59.999Z",
  root = "docs/universality/campaign-v6/u1-github-raw",
...
type State = {
  schema: "open-autonomy.github-census-window-state.v2";
  campaignId: string;
...
const commit = (
    await Bun.$`git rev-parse 66a48f3^{commit}`.quiet().text()
  ).trim(),
...
for (const path of [
  "scripts/universality/capture-u1-v6.ts",
  "packages/core/src/organization-universality-census.ts",
  "packages/core/src/organization-universality-window-contract.ts",
]) {
  if (
...
const show = (path: string) => {
  ...
  contract = verifySourceCensusWindowContract(
    show(
      "docs/universality/campaign-v6/source-census-contract.json",
    ) as FrozenSourceCensusWindowContract,
  ),
  claim = verifyFrozenUniversalityClaim(
    show("docs/universality/campaign-v6/claim.json") as FrozenUniversalityClaim,
  );
if (
...
const validatePass = async (attempt: Attempt) => {
  ...
  for (const ref of manifest.requests) {
    ...
    if (parsed.incomplete_results) throw Error("raw-evidence-invalid");
    ...
    if (url.searchParams.get("per_page") === "100")
      for (const item of parsed.items) {
        ...
        if (rawRows.has(item.node_id))
          throw Error("raw-evidence-duplicate-node");
  ...
  const rangeQuery = (range: CensusRange) =>
    [
      ...
      .filter(Boolean)
      .join(" ");
  const rootResponse = responses.get("stars:>=1000 fork:true|1|1"),
    maximum = rootResponse?.items[0]?.stargazers_count;
  ...
  const queue: CensusRange[] = [{ stars: [1000, maximum] }],
    ...
    expectedKeys: string[] = ["stars:>=1000 fork:true|1|1"];
  while (queue.length) {
    const range = queue.shift()!,
      countKey = `${rangeQuery(range)}|1|1`,
    ...
    for (let page = 1; page <= Math.ceil(count / 100); page++) {
      const pageKey = `${rangeQuery(range)}|100|${page}`,
        pageResponse = responses.get(pageKey);
      expectedKeys.push(pageKey);
      if (!pageResponse || pageResponse.total_count !== count)
        throw Error("raw-page-coverage-missing");
      for (const item of pageResponse.items)
        if (
          ...
          throw Error("raw-page-item-outside-leaf");
    }
  }
...
const finalizeIfReady = async () => {
  ...
  const population = "docs/universality/campaign-v6/u1-github-enumeration.json",
  ...
  const artifact = {
    schema: "open-autonomy.github-exhaustive-window-enumeration.v2",
    campaignId,
    registrationCommit: commit,
    captureImplementationCommit: implementationCommit,
    sourceCensusContractDigest: contract.digest,
...
async function runPass(attempt: Attempt) {
  ...
  const passStartedAt = attempt.startedAt,
    rootResult = (await request("stars:>=1000 fork:true", 1, 1)).value,
  ...
  while (queue.length) {
    const range = queue.shift()!,
      countResult = (await request(query(range), 1, 1)).value,
    ...
    for (let page = 1; page <= Math.ceil(count / 100); page++) {
      const { value, observedAt } = await request(query(range), 100, page);
      if (value.total_count !== count)
        throw new RetryPass("leaf-total-count-drift");
      for (const item of value.items) {
        if (
          item.stargazers_count < range.stars[0] ||
          item.stargazers_count > range.stars[1] ||
        ...
        if (
          range.created &&
          (!/^\d{4}-\d{2}-\d{2}T/.test(item.created_at) ||
            item.created_at.slice(0, 10) < range.created[0] ||
            item.created_at.slice(0, 10) > range.created[1])
        )
          throw new RetryPass("malformed-response-schema");
