...
import {
  ...
  ORGANIZATION_P3_OBLIGATIONS,
  ORGANIZATION_P4_OBLIGATIONS,
  ORGANIZATION_P5_OBLIGATIONS,
  ORGANIZATION_P6_OBLIGATIONS,
  ORGANIZATION_P7_OBLIGATIONS,
...
const publicSurfaceFiles = [
  ...
  'organization-migrate.ts',
  'organization-expression.ts',
  'organization-behavior.ts',
  'organization-component.ts',
  'organization-solver.ts',
...
describe('B0 semantic coverage and residual accounting', () => {
  ...
  test('accounts for every formal P3 obligation with evidence and no unresolved disposition', () => {
    const audit = readFileSync('docs/ORGANIZATION-IR-LENS-AUDIT.md', 'utf8');
    const documented = [...audit.matchAll(/^\| (P3-[A-Z]+-\d+) /gm)].map((match) => match[1]).sort();
    expect(ORGANIZATION_P3_OBLIGATIONS.map((item) => item.id).sort()).toEqual(documented);
    for (const obligation of ORGANIZATION_P3_OBLIGATIONS) {
      expect(obligation.disposition).not.toBe('unresolved');
      expect(obligation.evidence?.trim().length).toBeGreaterThan(0);
    }
  });

  test('accounts for every formal P4 obligation with evidence and no unresolved disposition', () => {
    const audit = readFileSync('docs/ORGANIZATION-IR-LENS-AUDIT.md', 'utf8');
    const documented = [...audit.matchAll(/^\| (P4-[A-Z]+-\d+) /gm)].map((match) => match[1]).sort();
    expect(ORGANIZATION_P4_OBLIGATIONS.map((item) => item.id).sort()).toEqual(documented);
    for (const obligation of ORGANIZATION_P4_OBLIGATIONS) {
      expect(obligation.disposition).not.toBe('unresolved');
      expect(obligation.evidence?.trim().length).toBeGreaterThan(0);
    }
  });

  test('accounts for every formal P5 obligation with evidence and no unresolved disposition', () => {
    const audit = readFileSync('docs/ORGANIZATION-IR-LENS-AUDIT.md', 'utf8');
    const documented = [...audit.matchAll(/^\| (P5-[A-Z]+-\d+) /gm)].map((match) => match[1]).sort();
    expect(ORGANIZATION_P5_OBLIGATIONS.map((item) => item.id).sort()).toEqual(documented);
    for (const obligation of ORGANIZATION_P5_OBLIGATIONS) {
      expect(obligation.disposition).not.toBe('unresolved');
      expect(obligation.evidence?.trim().length).toBeGreaterThan(0);
    }
  });

  test('accounts for every formal P6 obligation with evidence and no unresolved disposition', () => {
    const audit = readFileSync('docs/ORGANIZATION-IR-LENS-AUDIT.md', 'utf8');
    const documented = [...audit.matchAll(/^\| (P6-[A-Z]+-\d+) /gm)].map((match) => match[1]).sort();
    expect(ORGANIZATION_P6_OBLIGATIONS.map((item) => item.id).sort()).toEqual(documented);
    for (const obligation of ORGANIZATION_P6_OBLIGATIONS) {
      expect(obligation.disposition).not.toBe('unresolved');
      expect(obligation.evidence?.trim().length).toBeGreaterThan(0);
    }
  });

  test('accounts for every formal P7 obligation with evidence and no unresolved disposition', () => {
    const audit = readFileSync('docs/ORGANIZATION-IR-LENS-AUDIT.md', 'utf8');
    const documented = [...audit.matchAll(/^\| (P7-[A-Z]+-\d+) /gm)].map((match) => match[1]).sort();
    expect(ORGANIZATION_P7_OBLIGATIONS.map((item) => item.id).sort()).toEqual(documented);
    for (const obligation of ORGANIZATION_P7_OBLIGATIONS) {
      expect(obligation.disposition).not.toBe('unresolved');
      expect(obligation.evidence?.trim().length).toBeGreaterThan(0);
    }
  });

  test('accounts for every formal P8 obligation with evidence and no unresolved disposition', () => {
    const audit = readFileSync('docs/ORGANIZATION-IR-LENS-AUDIT.md', 'utf8');
    const documented = [...audit.matchAll(/^\| (P8-[A-Z]+-\d+) /gm)].map((match) => match[1]).sort();
    expect(ORGANIZATION_P8_OBLIGATIONS.map((item) => item.id).sort()).toEqual(documented);
    for (const obligation of ORGANIZATION_P8_OBLIGATIONS) {
      expect(obligation.disposition).not.toBe('unresolved');
      expect(obligation.evidence?.trim().length).toBeGreaterThan(0);
    }
  });

  test('accounts for every formal P9 obligation with evidence and no unresolved disposition', () => {
    const audit = readFileSync('docs/ORGANIZATION-IR-LENS-AUDIT.md', 'utf8');
    const documented = [...audit.matchAll(/^\| (P9-[A-Z]+-\d+) /gm)].map((match) => match[1]).sort();
    expect(ORGANIZATION_P9_OBLIGATIONS.map((item) => item.id).sort()).toEqual(documented);
    for (const obligation of ORGANIZATION_P9_OBLIGATIONS) {
      expect(obligation.disposition).not.toBe('unresolved');
      expect(obligation.evidence?.trim().length).toBeGreaterThan(0);
    }
  });

  test('accounts for every formal P10 obligation with evidence and no unresolved disposition', () => {
    const audit = readFileSync('docs/ORGANIZATION-IR-LENS-AUDIT.md', 'utf8');
  ...
  test('accounts for every formal P12 obligation with evidence and no unresolved disposition', () => {
    const audit = readFileSync('docs/ORGANIZATION-IR-LENS-AUDIT.md', 'utf8');
