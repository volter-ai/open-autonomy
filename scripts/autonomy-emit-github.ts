...
function workflowYml(wf: IRWorkflow): string {
  ...
  return [
    ...
    `          bun scripts/model-proxy-mint.ts --run-id "ir-${wf.launch}-\${{ github.run_id }}" --models "$MODEL" --max-usd-cents 50 --max-requests 30 --issue /tmp/issue.json`,
    `      - name: run the agent (real model call through the proxy, responses API)`,
    `        run: |`,
    `          RESP=$(curl -sS "$MODEL_PROXY_URL/openai/v1/responses" -H "authorization: Bearer \${{ steps.mint.outputs.token }}" -H "content-type: application/json" -d "$(jq -nc --arg m "$MODEL" --arg t "$TASK" '{model:$m,input:($t+" Output ONLY the file content, nothing else.")}')")`,
    `          echo "raw response (first 400 chars): \${RESP:0:400}"`,
    `          echo "$RESP" | jq -r '.output_text // ([.output[]?.content[]?|select(.type=="output_text").text]|join(""))' > IR-AGENT-PROOF.md`,
