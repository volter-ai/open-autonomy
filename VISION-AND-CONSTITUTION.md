...
|---|---|---|
| **Vision** | `docs/VISION.md` + `docs/CONSTITUTION.md` §North Star/§Method. **Correction (second pass):** `VISION.md` is *not* in the profile's `resources:` list and does **not** ship to installs — only `CONSTITUTION`/`PROJECT`/`ARCHITECTURE`/`ROADMAP` are seeded (`profiles/self-driving/ir.yml:180-219`, `INSTALL_OWNED_PATHS` in `upgrade.ts:17-35`). The Vision is the *project's*; each installation's direction anchor is its seeded Constitution + Project charter. | The IR touchpoint for direction docs is `AutonomyIR.resources: string[]` (seed) + `INSTALL_OWNED_PATHS` (seed-once, human-owned thereafter). Direction, not machinery. |
...
**The through-line:** Vision → the project's own prose (repo-owned; installs get the seeded Constitution + Project charter instead). Constitution → prose + the `validateIR` checks + the policy safety keys + the invariants file (governance, partly enforced). Roadmap → `roadmap.yml`'s two layers, gated by `proof_gate` (work). SDLC → the compiled `agents:` graph (execution). Each distilled doc in this effort is written to inform its real counterpart, and the roadmap's top items (Part B.4) are precisely the gaps between what these documents *assert* and what the IR *enforces*.

### C.1 How these documents enter the installation (the entry paths)

Because this repo *is* `compile(profiles/self-driving, github)`, this document set must not become a parallel canon of unclassified root files — that would reproduce the two-lanes pattern this review diagnoses (§I.3, B.4 #19). Each artifact has exactly one legitimate entry path into the installation's own document model:

| This artifact | Enters the installation as | Via which seam |
|---|---|---|
| Part 0 creed + Part B invariants | Amendments to the install-owned `docs/CONSTITUTION.md`; enforceable rows as `.open-autonomy/architecture-invariants.yml` entries | The human-required path (both files trip the human-approval gate) — the amendment rule demonstrating itself |
| `ROADMAP-DISTILLED.md` items (OA-2..10) | `proposed:` layer-1 items in `.open-autonomy/roadmap.yml` (each already carries its `roadmap.v2 slot`) | The strategist seam: this review *is* an out-of-band strategist run; ratification (proposed→planned) is the human step; the planner then reaps issues and the fleet burns them. The ztrack store is the reviewer's staging tracker and retires as items are ratified into the org's own two-layer tracker. |
| `ARCHITECTURE-REVIEW.md` / `SDLC-ASSESSMENT.md` | Audit *records* (the strategist-archive pattern), plus their factual corrections drive the OA-6 edits to the install-owned `docs/ARCHITECTURE.md` and the repo-owned `SPEC.md` | Review-record archive + normal gated PRs |

The end state that counts as "done" for this whole exercise: the content has flowed through the org's own governance into the seeded/owned documents, and these root files remain only as the audit trail of how it got there.
