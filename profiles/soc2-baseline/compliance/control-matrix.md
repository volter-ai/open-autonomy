...
| C3 | Egress lockdown (exfiltration prevention) | CC6 | `harden-runner egress-policy: block` on every credentialed job. **Enforces FREE on PUBLIC repos (no StepSecurity account)** — the shipped allowlist now includes harden-runner's own agent endpoints (`agent.api.stepsecurity.io`/`prod.app-api.stepsecurity.io`), without which the block agent can't initialize and fails open. **Verified live, no account: PUBLIC repo → example.com DENIED (curl exit 7); PRIVATE repo → still ALLOWED.** PRIVATE repos need **StepSecurity Enterprise** (public/private boundary, like GHAS). A naive self-managed iptables block breaks the runner control-plane (verified) — not a drop-in. | harden-runner step + run logs (exit 7 on a blocked host) |
