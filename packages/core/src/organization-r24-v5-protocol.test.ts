...
function fixture(kind: "exited" | "timed-out" = "exited") {
  ...
  return {
    ...
    binding,
    configuration: { digest: d, rawDigest: d, readbackAt: "2026-07-15T00:00:00Z" },
    ...
    supervisor: {
      ...
      containment: { kind: "provider-supervised-tree", id: "cell", owner: "provider-supervisor", killPrimitive: "provider-tree-kill", escapePreventionEvidence: "native provider process tree" },
      timeout: { deadlineAt: "2026-07-15T00:00:02Z", graceMs: 100 },
      supervisor: { identity: { pid: 1, pgid: 1, startTicks: "1", bootId: "boot", role: "supervisor", parentPid: null, containmentId: "cell" }, diedAt: null },
      processes: [
        { pid: 12, pgid: 12, startTicks: "12", bootId: "boot", role: "launcher", parentPid: 1, containmentId: "cell" },
        { pid: 20, pgid: 12, startTicks: "20", bootId: "boot", role: "hermes", parentPid: 12, containmentId: "cell" },
        { pid: 30, pgid: 12, startTicks: "30", bootId: "boot", role: "tool", parentPid: 20, containmentId: "cell" },
      ],
      groupRelation: "shared",
      observations: [{ at: "2026-07-15T00:00:01Z", source: "procfs+provider", members: [
        { pid: 12, pgid: 12, startTicks: "12", bootId: "boot", role: "launcher", parentPid: 1, containmentId: "cell" },
        { pid: 20, pgid: 12, startTicks: "20", bootId: "boot", role: "hermes", parentPid: 12, containmentId: "cell" },
        { pid: 30, pgid: 12, startTicks: "30", bootId: "boot", role: "tool", parentPid: 20, containmentId: "cell" },
      ], populated: true, evidenceDigest: d }],
      signals: [{ kind: "TERM", at: "2026-07-15T00:00:02Z", targetContainmentId: "cell", authority: "provider-supervisor", evidenceDigest: d }, { kind: "KILL", at: "2026-07-15T00:00:03Z", targetContainmentId: "cell", authority: "provider-supervisor", evidenceDigest: d }],
      terminal: [12,20,30].map((pid, i) => ({ identity: [
        { pid: 12, pgid: 12, startTicks: "12", bootId: "boot", role: "launcher", parentPid: 1, containmentId: "cell" },
        { pid: 20, pgid: 12, startTicks: "20", bootId: "boot", role: "hermes", parentPid: 12, containmentId: "cell" },
        { pid: 30, pgid: 12, startTicks: "30", bootId: "boot", role: "tool", parentPid: 20, containmentId: "cell" },
      ][i]!, at: "2026-07-15T00:00:03Z", kind: "signaled", status: 9, evidenceDigest: d })),
      final: { at: "2026-07-15T00:00:04Z", populated: false, members: [], reap: [12,20,30].map((pid, i) => ({ identity: [
        { pid: 12, pgid: 12, startTicks: "12", bootId: "boot", role: "launcher", parentPid: 1, containmentId: "cell" },
        { pid: 20, pgid: 12, startTicks: "20", bootId: "boot", role: "hermes", parentPid: 12, containmentId: "cell" },
        { pid: 30, pgid: 12, startTicks: "30", bootId: "boot", role: "tool", parentPid: 20, containmentId: "cell" },
      ][i]!, proof: "provider-reaped", evidenceDigest: d })), evidenceDigest: d },
    },
    ...
    preservation: [{ source: "native", target: "canonical", disposition: "normalized", rationale: "fixture" }],
  } as V5NativeTrace;
}
...
describe("R24 V5 authenticated causal replay", () => {
  ...
  test("rejects duplicate/forged receipt, PID mismatch, swapped or ambiguous outer run, living descendant and terminal disagreement", () => {
    for (const mutate of [
      ...
      (x: V5NativeTrace) => (x.supervisor.processes.find(p=>p.role==="hermes")!.pid = 21),
