...
import { activationHome, readActivationRoutingState } from './activation-paths.ts';
...
export function checkUncommittedHarness(cwd: string, proc: ProcRunner = defaultProc, env: NodeJS.ProcessEnv = process.env): GuardResult {
  ...
  // Activation generations are validated deployment artifacts, not ordinary authoring worktrees. They
  // may intentionally differ from the portable accepted commit by machine-local provider/schedule
  // materialization. Trust only the exact routed root+SHA and only while every generated byte still
  // matches the receipt captured during activation validation. A timestamp alone would let a later edit
  // inherit stale trust.
  const declaredHome = (env.AUTONOMY_ACTIVATION_HOME ?? '').trim();
  const declaredRoot = (env.AUTONOMY_CONTROL_ROOT ?? '').trim();
  const declaredSha = (env.AUTONOMY_CONTROL_SHA ?? '').trim();
  if (declaredHome && declaredRoot && /^[0-9a-f]{40}$/.test(declaredSha)) {
    try {
      const home = activationHome(cwd, proc);
      const state = readActivationRoutingState(cwd, proc);
      const candidates = [state?.active, ...(state?.draining ?? [])].filter((generation) => generation !== undefined);
      const generation = candidates.find((candidate) => candidate.sha === declaredSha);
      const expectedRoot = join(home, 'generations', declaredSha);
      const digest = generation && 'validationDigest' in generation ? generation.validationDigest : undefined;
      if (
        canonicalPath(declaredHome) === canonicalPath(home) &&
        canonicalPath(cwd) === canonicalPath(expectedRoot) &&
        canonicalPath(declaredRoot) === canonicalPath(expectedRoot) &&
        generation &&
        canonicalPath(generation.root) === canonicalPath(expectedRoot) &&
        generation.validatedAt &&
        typeof digest === 'string' &&
        generationValidationDigest(cwd) === digest
      ) return { ok: true };
    } catch {
      // Ordinary refusal below is the fail-closed outcome for corrupt/missing state or receipt mismatch.
    }
  }
