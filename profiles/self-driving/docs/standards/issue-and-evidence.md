...
Read this from `draft`, `develop`, and `reviewer` — the ztrack acceptance-criteria grammar this profile
uses to gate issue content. Converged from simple-gh-sdlc's `standards/issue-and-evidence.md` (supercode
study §II.8.1 row 3: self-driving adopts sdlc's ztrack preset — `policy.tracker.ztrackPreset` in
`ir.yml` — so the dogfood finally grades its own issues with its own tracker's grammar, exactly like every
other ztrack-tracked profile). This file is kept at a self-driving-specific path (`docs/standards/`, not
sdlc's bare `standards/`) so it does not collide on install path with simple-gh-sdlc's copy — `check:profiles`
enforces byte-identity for any path 2+ github profiles carry, and the two profiles' wording legitimately
differs (this one names self-driving's own skills and origin, including roadmap-originated issues).

Issues in scope: both a `draft`-shaped human request AND a planner-authored roadmap tracking issue
(`roadmap:<id>` label) — either way, once an issue carries ACs in its body, the grammar below is what
`develop` implements against and `reviewer` gates on.
