...
- agent-runtime scripts: `claude-agent-run.ts` + `agent.ts` (the credentialed skill runner),
  ...
  `agent-propose.ts`, `finalize-agent-review.ts`, plus the substrate-neutral `review-result.ts` protocol
  parser shared with trusted consumers — ONLY actor-execution machinery and its standard result contract (the runner: which box,
