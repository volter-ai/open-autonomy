...
- [ ] dev/07 v1 The CODE_HOST_RESOURCES split is completed at the script layer (audit §2.2): `rearm-auto-merge.ts`, `human-approval-gate.ts`, and `reconcile-merged-issues.ts` move from the substrate's vendored runtime into profile-carried resources (the `profiles/self-driving/scripts/` + `resources:` mechanism), leaving the runtime mirror launch/credential/lifecycle-only; pairs with OA-5 dev/09 (the moved scripts enter `human_required_paths`).
