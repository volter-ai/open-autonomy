...
| C4 | Human-approval change gate | CC8, **PI** | `human-approval.yml` + `scripts/human-approval-gate.ts`, per-head-SHA maintainer authorization (native Approve or exact-SHA `/agent approve`) on human-required paths | `human-approval` status; review/comment records |
