...
| **M4** | **`policy.box.risk` → human-required gate** | `human_required_paths` → materialized to `.open-autonomy/human-required-paths.json`, enforced by the deterministic `human-approval` gate (per-head-SHA maintainer approve). Semantic stop topics remain agent methodology in the risk standard. | `emit.ts` (materialize), `scripts/human-approval-gate.ts` (deterministic gate). **Deterministic change-management control over sensitive paths.** |
...
| **M6** | **Provisioning → branch protection / repo settings** | Required status checks, `enforce_admins`, required PR reviews, (and settable: required signatures, secret-scanning, push-protection) | `scripts/provision-target-repo.ts:350-356` sets `required_status_checks`, `enforce_admins:false`, `required_pull_request_reviews`, `restrictions:null`. Required checks come from a **provision manifest** (`branch_protection.required_checks`, today `["ci","agent-review"]` via `bench/provision.template.json:31-34`). **This is where SOC 2 branch-protection hardening lands — but the manifest is not yet profile-derived (see §3 gap G1).** `allow_auto_merge=true` (`:340`) is **opt-in only** (`--arm-auto-merge`, TE.10) — never fired by `oa install`'s automated EXECUTE; arming it is a deliberate, human-gated LATER step per TE.6's G4b runbook. |
...
(`provision-target-repo.ts:350-356`).
