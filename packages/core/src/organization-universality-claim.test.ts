...
test("U0 committed campaign artifact is an authentic frozen registration",()=>{
  const artifact=JSON.parse(readFileSync("docs/universality/campaign-v8/claim.json","utf8")) as FrozenUniversalityClaim;
...
test("U0 preserves and explicitly invalidates every predecessor instead of rewriting history",()=>{
  ...
  const v6=JSON.parse(readFileSync("docs/universality/campaign-v6/claim.json","utf8")) as FrozenUniversalityClaim;
  const v7=JSON.parse(readFileSync("docs/universality/campaign-v7/claim.json","utf8")) as FrozenUniversalityClaim;
  const v8=JSON.parse(readFileSync("docs/universality/campaign-v8/claim.json","utf8")) as FrozenUniversalityClaim;
  ...
  const s56=JSON.parse(readFileSync("docs/universality/campaign-v6/supersession.json","utf8")) as FrozenCampaignSupersession;
  const s67=JSON.parse(readFileSync("docs/universality/campaign-v7/supersession.json","utf8")) as FrozenCampaignSupersession;
  const s78=JSON.parse(readFileSync("docs/universality/campaign-v8/supersession.json","utf8")) as FrozenCampaignSupersession;
  expect(verifyHistoricalUniversalityClaimDigest(v1)).toEqual(v1); expect(verifyHistoricalUniversalityClaimDigest(v2)).toEqual(v2); expect(verifyHistoricalUniversalityClaimDigest(v3)).toEqual(v3); expect(verifyHistoricalUniversalityClaimDigest(v4)).toEqual(v4); expect(verifyHistoricalUniversalityClaimDigest(v5)).toEqual(v5); expect(verifyHistoricalUniversalityClaimDigest(v6)).toEqual(v6); expect(verifyHistoricalUniversalityClaimDigest(v7)).toEqual(v7); expect(verifyFrozenUniversalityClaim(v8)).toEqual(v8);
  expect(verifyFrozenCampaignSupersession(s12,v1,v2)).toEqual(s12); expect(verifyFrozenCampaignSupersession(s23,v2,v3)).toEqual(s23); expect(verifyFrozenCampaignSupersession(s34,v3,v4)).toEqual(s34); expect(verifyFrozenCampaignSupersession(s45,v4,v5)).toEqual(s45); expect(verifyFrozenCampaignSupersession(s56,v5,v6)).toEqual(s56); expect(verifyFrozenCampaignSupersession(s67,v6,v7)).toEqual(s67); expect(verifyFrozenCampaignSupersession(s78,v7,v8)).toEqual(s78);
  expect(Date.parse(v8.registeredAt)).toBeLessThan(Date.parse(v8.censusAt));
});
...
test("U0 closure is machine indexed, skeptic accepted, residual-free, and advances only to U1",()=>{
  const closure=JSON.parse(readFileSync("docs/universality/campaign-v8/u0-closure.json","utf8"));
  const claim=JSON.parse(readFileSync("docs/universality/campaign-v8/claim.json","utf8")) as FrozenUniversalityClaim;
...
test("U0 verifier rejects reversed chronology and forged or surplus supersession joins",()=>{
  ...
  const oldClaim=JSON.parse(readFileSync("docs/universality/campaign-v7/claim.json","utf8")) as FrozenUniversalityClaim;
  const newClaim=JSON.parse(readFileSync("docs/universality/campaign-v8/claim.json","utf8")) as FrozenUniversalityClaim;
  const value:any=JSON.parse(readFileSync("docs/universality/campaign-v8/supersession.json","utf8")); delete value.digest;
