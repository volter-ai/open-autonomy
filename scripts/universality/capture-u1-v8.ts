...
import {
  verifySourceCensusIndexObservationContract,
  type FrozenSourceCensusIndexObservationContract,
} from "../../packages/core/src/organization-universality-index-observation-contract";
import {
  verifyFrozenUniversalityClaim,
  type FrozenUniversalityClaim,
} from "../../packages/core/src/organization-universality-claim";
const campaignId = "organization-universality-2026-v8",
  cutoff = "2026-07-29T23:59:59.999Z",
  root = "docs/universality/campaign-v8/u1-github-raw",
...
const commit = (
    await Bun.$`git rev-parse fb6a04c^{commit}`.quiet().text()
  ).trim(),
...
for (const path of [
  "scripts/universality/capture-u1-v8.ts",
  "packages/core/src/organization-universality-census.ts",
  "packages/core/src/organization-universality-index-observation-contract.ts",
]) {
  if (
...
const show = (path: string) => {
  ...
  contract = verifySourceCensusIndexObservationContract(
    show(
      "docs/universality/campaign-v8/source-census-contract.json",
    ) as FrozenSourceCensusIndexObservationContract,
  ),
  claim = verifyFrozenUniversalityClaim(
    show("docs/universality/campaign-v8/claim.json") as FrozenUniversalityClaim,
  );
if (
...
const canonicalObservationDigest = (observations: unknown) =>
  `sha256:${createHash("sha256")
    .update(
      `open-autonomy.u1.query-observations.v1\0${canonicalSemanticJson(observations)}`,
    )
    .digest("hex")}`;
const passObservations = (rows: any[], pass: number) =>
  rows
    .map((row) => ({
      nodeId: row.nodeId,
      pass,
      observedAt: row.observedAt,
      queryStars: row.queryStars,
      queryCreated: row.queryCreated,
    ...
    .sort(
      (a, b) =>
        ...
        canonicalSemanticJson(a.queryStars).localeCompare(
          canonicalSemanticJson(b.queryStars),
        ) ||
        canonicalSemanticJson(a.queryCreated).localeCompare(
          canonicalSemanticJson(b.queryCreated),
        ),
    );
...
const validatePass = async (attempt: Attempt) => {
  ...
  if (
    ...
    value.observationDigest !== attempt.observationDigest ||
  ...
  for (const row of value.repositories) {
    if (
      !row.nodeId ||
      ids.has(row.nodeId) ||
      !Number.isSafeInteger(row.returned?.stars) ||
      row.returned.stars < 1000 ||
      !Array.isArray(row.queryStars) ||
      row.queryStars.length !== 2 ||
      !(
        row.queryCreated === null ||
        (Array.isArray(row.queryCreated) && row.queryCreated.length === 2)
      ) ||
    ...
    tuples.push({
      node_id: row.nodeId,
      full_name: row.returned.repository,
      stargazers_count: row.returned.stars,
      fork: row.returned.fork,
      archived: row.returned.archived,
    });
  }
  if (
    censusTupleDigest(tuples) !== value.tupleDigest ||
    leafCountDigest(Object.entries(value.leafCounts)) !==
      value.leafCountDigest ||
    canonicalObservationDigest(
      passObservations(value.repositories, attempt.attempt),
  ...
  for (;;) {
    const key = `stars:>=${threshold} fork:true|100|1`,
      response = responses.get(key);
    expectedKeys.push(key);
    if (!response) throw Error("raw-tail-probe-missing");
    if (response.parsed.total_count === 0) {
      if (response.parsed.items.length !== 0)
        throw Error("raw-tail-probe-inconsistent-empty");
      maximum = threshold - 1;
      break;
    }
    if (
      !Number.isSafeInteger(response.parsed.total_count) ||
      threshold > Number.MAX_SAFE_INTEGER / 2
    )
      throw Error("raw-tail-probe-invalid");
    threshold *= 2;
  }
  ...
  while (queue.length) {
    ...
    for (const item of countResponse.parsed.items) {
      ...
      const row = {
        ...
        queryStars: range.stars,
        queryCreated: range.created ?? null,
        returned: {
...
const finalizeIfReady = async () => {
  ...
  const repositories = [...byNode.entries()]
    ...
    .map(([nodeId, observations]) => ({
      nodeId,
      canonicalAdoptionValue: Math.max(
        ...observations.map((x) => x.returned.stars),
      ),
      observations: observations.sort(
        (a, b) => a.pass - b.pass || a.observedAt.localeCompare(b.observedAt),
      ),
    }));
  const combinedObservationDigest = canonicalObservationDigest(
      repositories.flatMap((row) => row.observations),
    ),
    population = "docs/universality/campaign-v8/u1-github-enumeration.json",
  ...
  const artifact = {
    schema: "open-autonomy.github-exhaustive-index-observation-enumeration.v4",
    campaignId,
    registrationCommit: commit,
    captureImplementationCommit: implementationCommit,
    sourceCensusContractDigest: contract.digest,
    ...
    observationDigest: combinedObservationDigest,
...
async function runPass(attempt: Attempt) {
  ...
  let threshold = 2000,
    maximum: number;
  for (;;) {
    const probe = (await request(`stars:>=${threshold} fork:true`, 100, 1))
      .value;
  ...
  while (queue.length) {
    ...
    for (const item of response.value.items) {
      if (item.stargazers_count < 1000 || repositories.has(item.node_id))
        throw new RetryPass(
  ...
  const rows = [...repositories.values()].sort((a, b) =>
    ...
    tuples: CensusTuple[] = rows.map((r) => ({
      node_id: r.nodeId,
      full_name: r.returned.repository,
      stargazers_count: r.returned.stars,
      fork: r.returned.fork,
      archived: r.returned.archived,
    })),
  ...
  const result = `${dir}/accepted-pass.json`,
    ...
    observationDigest = canonicalObservationDigest(
      passObservations(rows, attempt.attempt),
    ),
